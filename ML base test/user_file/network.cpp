	...
	while (true){
		...
		for (i = 0; i < tr->outmat[0]->line; ++i) {														//更新隐藏层权重				
			...
			for (j = 0; j < tr->layer[0]->row;++j) {
				layer0[offset0 + j] -= tr->bias_speed[2] * out_network_dersum*matadd[j];				//hide_network_j=learm_speed*outdersum*mat_j
			}
		...
		for (i = 0; i < tr->layer[1]->line; ++i) {													//更新输出层权重
			for (j = 0; j < tr->layer[1]->row; ++j) {
				layer1[offset1 + j] -= tr->bias_speed[2] * error_mat1[i] * outmat0[j];
			}
			offset1 += tr->layer[1]->row;
		}																								//更新输出层完成
		printf("迭代次数 = %d\n\n", update++);
...
float forward_propaga(TrStr *tr)
{
	...
	mat_addto_value(tr->outmat[0], tr->bias_speed[0]);
	...
	mat_addto_value(tr->outmat[1], tr->bias_speed[1]);
...
//ac_fi0:隐藏层激活函数标志
//ac_fi1:输出层激活函数标志
bool Train_init(TrStr *tr,MatStr *mat,MatStr *hide_mat,MatStr *end_mat,float *bias_Lspeed,uint8_t ac_fi0,uint8_t ac_fi1)
{
	//初始化输入矩阵mat
	tr->mat = mat;		
	//初始化隐藏层(0)&输出层网络矩阵(1)
	...
	tr->bias_speed = bias_Lspeed;
	//初始化隐藏层输出矩阵(0)&输出层矩阵(1)
	tr->outmat[0] = mat_create(hide_mat->line, mat->row, f32Flag);
