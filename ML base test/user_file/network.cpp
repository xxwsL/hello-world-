...
bool back_propaga(const MatStr *mat,MatStr *hide_network_mat,MatStr *out_nework_mat,const MatStr *target_mat,float learm_speed)
{
	...
	float *out_matadd = NULL, *error_matadd = NULL, *out_nework_matadd = NULL, *neth_matadd = NULL;
	...
	while (true){
		...
		for (i = 0; i < out_nework_mat->line; ++i) {													//计算输出层误差
			*(out_matadd +i) = (-(*(error_matadd + i)))*active_derfi(*(out_matadd+i));						//error_der=-(traget_i-out_i)*outi(1-out_i)
		}																
		for (i = 0; i < neth_mat->line; ++i) {															//更新隐藏层权重				
			for (j = 0; j < out_nework_mat->line;++j) {
				out_network_dersum += (*(out_matadd + j))*(*(out_nework_matadd + offset1 + i));			//out_dersum+=error_der*Wout_network
		...
		for (i = 0; i < out_nework_mat->line; ++i) {										//更新输出层权重
			for (j = 0; j < out_nework_mat->row; ++j) {
				*(out_nework_matadd + offset1 + j) -= learm_speed*(*(out_matadd +i))*(*(neth_matadd + j));
			}
			offset1 += out_nework_mat->row;
		}																					//更新输出层完成
		printf("迭代次数 = %d\n\n", update++);
